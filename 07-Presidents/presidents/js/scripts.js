...
for (let x = 0; x < presidents.length; x++) {
    ...
    presidentName.innerHTML = `<p>${presidents[x].name}</p>`
    ...
    presidentCaption.innerHTML = `Served from ${presidents[x].took_office} - ${presidents[x].left_office} <br> Lived from ${presidents[x].born} to ${presidents[x].died} <br> Party: ${presidents[x].party}`
